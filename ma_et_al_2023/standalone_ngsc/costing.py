...
case_list = ['smr_ngsc_ccs','smr_ngcc_ccs','smr_ngsc','smr_ngcc','smr','ngcc','ngsc']
...
def LCOE_cal(summary, product,cf, case = 'smr'):
    LCOE = pd.DataFrame()
    if case == 'ngcc':
        power = 369
    elif case == 'ngsc':
        power = 114
    LCOE['Value, $/MWh'] = (summary.iloc[:,0]/(power*365*24/1000000*cf)).round(2)
    LCOE.at['Total','Value, $/MWh'] = LCOE['Value, $/MWh'].sum()
...
def cost_sum(product,total,cost,case,cf):
