...
tube side flows from 1 to 0"""))
    ...
    def _make_performance(self):
        ...
        if self.config.has_radiation == True:
            ...
            def gas_emissivity_eqn(b, t, x):
                ...
                X4 = b.shell.properties[t, x].mole_frac['CO2']
                X5 = b.shell.properties[t, x].mole_frac['H2O']
                X6 = b.shell.properties[t, x].mole_frac['O2']
        ...
        def N_Re_shell_eqn(b, t, x):
            return b.N_Re_shell[t,x] * b.shell.properties[t,x].visc_d == \
                   b.do_tube * b.v_shell[t,x] * b.shell.properties[t,x].dens_mol_phase["Vap"]*b.shell.properties[t, x].mw
        ...
        def deltaP_shell_eqn(b, t, x):
            return b.deltaP_shell[t,x]*b.pitch_x == -1.4 * b.friction_factor_shell[t,x] * \
                   b.shell.properties[t,x].dens_mol_phase["Vap"] * b.shell.properties[t, x].mw \
                   * b.v_shell[t,x]**2
    ...
    def initialize_build(blk, shell_state_args=None, tube_state_args=None, outlvl=0,
    ...
    def calculate_scaling_factors(self):
        ...
        for i, c in self.heat_holdup_eqn.items():
            sf = iscale.get_scaling_factor(
                self.heat_holdup[i], default=1, warning=True)
            iscale.constraint_scaling_transform(c, sf, overwrite=False)